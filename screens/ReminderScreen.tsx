...
type ExpiredItem = {
    id: string;
    itemName: string; // Changed from 'name' to 'itemName' to match your subcollection structure
    expiryDate: string; // Changed from 'date' to 'expiryDate'
    uid: string;
};
...
const ReminderScreen: React.FC<Props> = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchExpiredItems = async () => {
            if (currentUser) {
                try {
                    const expiredItemsCollection = collection(db, 'users', currentUser.uid, 'expireItems');
                    const querySnapshot = await getDocs(expiredItemsCollection);
                    const fetchedItems: ExpiredItem[] = [];
    ...
    const deleteItem = async (id: string) => {
        if (currentUser) {
            try {
                const itemDocRef = doc(db, 'users', currentUser.uid, 'expireItems', id);
    ...
    const addToShoppingList = (id: string) => {
        ...
        // Implement your logic to add the item to the shopping list
    };
